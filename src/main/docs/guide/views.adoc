...
<div class="centered" style="margin: 10px auto;"> //<1>
        ...
        <span>|</span>
        <sec:ifNotLoggedIn>
            <g:render template="/login/loginWithFacebook"/> //<2>
        </sec:ifNotLoggedIn>
        <sec:ifLoggedIn>
...
<g:if test="${flash.message}"> //<4>
    <div class="message" style="display: block">${flash.message}</div>
</g:if>
----
<1> Center our menu; link to list all books; link to list favourite books, login with facebook / logout
...
We create a template to contain our link to trigger our OAuth login through Facebook; the link `/oauth/authenticate`
 is provided from the `spring-security-rest` plugin and ends with the provider we are using `/facebook` for the link
  `/oauth/authenticate/twitter`. This will redirect you to the normal Facebook login / account selection you are
  already familiar with.

[source,html]
.grails-app/views/login/_loginWithFacebook.gsp
----
include::{sourceDir}/grails-app/views/login/_loginWithFacebook.gsp[]
----

NOTE: `facebookSignin.png` is already provided for you in assets